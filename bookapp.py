...
def api_login():
    ...
    if result is not None:
       ...
       payload = {
          'user_id': result['user_id'], 'user_email': result['user_email'],
...
def api_valid():
   ...
   try:
      ...
      # payload 안에 email이 들어있습니다. 이 email로 유저정보를 찾습니다.
...
def check_info():
    ...
    try:
        ...
        count_no = db.books.count({'user_id':user_id})+1
...
def listing():
    ...
    try:
        ...
        # user_email_receive 클라이언트가 준 user_email_receive를 가져오기
        user_email_receive = payload['user_email']
        # user_email_receive의 값이 받은 user_email_receive와 일치하는 document 찾기 & _id 값은 출력에서 제외하기
        result = list(db.books.find({'user_email': user_email_receive},{'_id':0}))
        # books라는 키 값으로 정보 내려주기
